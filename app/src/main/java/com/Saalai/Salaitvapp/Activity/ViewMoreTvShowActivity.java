...
public class ViewMoreTvShowActivity extends AppCompatActivity {

    private static final String TAG = "ViewMoreTvShowActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_more_tv); // reuse same layout
        ...
        if (getIntent() != null) {
            ...
            TextView titleText = findViewById(R.id.txtCategoryTitle);
            if (categoryName != null) titleText.setText(categoryName);

            Log.d(TAG, "➡️ categoryId=" + categoryId + ", categoryName=" + categoryName);
        }
    ...
    private void fetchTvShows() {
        ...
        Log.d(TAG, "📡 Fetching TV shows: categoryId=" + categoryId + ", offset=" + offset + ", limit=" + limit);

        apiService.getTvShowList(accessToken, categoryId, offset, limit).enqueue(new Callback<ResponseBody>() {
            @Override
                    ...
                    try {
                        if (response.errorBody() != null) {
                            ...
                            if (jsonObject.has("error") && "access_denied".equals(jsonObject.getString("error"))) {
                                Toast.makeText(ViewMoreTvShowActivity.this, "Access Denied", Toast.LENGTH_SHORT).show();
                                callLogoutApi();
                            } else {
    ...
    private void callLogoutApi() {
        SharedPrefManager sp = SharedPrefManager.getInstance(ViewMoreTvShowActivity.this);
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    ...
                    Intent intent = new Intent(ViewMoreTvShowActivity.this, SignUpActivity.class);
    ...
    private void setupCloseReceiver() {
        closeReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if ("CLOSE_ViewMoreTvShowActivity".equals(intent.getAction())) {
                    finish();
                }
            }
        };

        IntentFilter filter = new IntentFilter("CLOSE_ViewMoreTvShowActivity");
